...
st.title("AI-Powered Predictive Analytics for Stock Market (Research Preview) by Praveen Dilaksha")
st.caption("Analyze stock data using **Linear Regression**, **Random Forest**, **XGBoost**, or **Prophet** — now with Kaggle dataset upload support!")
...
# --- Prophet Shortcut ---
model_choice = st.sidebar.selectbox("Select Model:", ["Linear Regression", "Random Forest", "XGBoost", "Prophet"])

if model_choice == "Prophet":
    if "Date" not in df.columns:
        st.error("Prophet requires a 'Date' column in the dataset.")
        st.stop()

    prophet_df = df[["Date", target_col]].rename(columns={"Date": "ds", target_col: "y"})
    prophet_df.dropna(inplace=True)

    forecast_days = st.sidebar.slider("Forecast Days Ahead", min_value=30, max_value=365, value=90)
    ...
    if st.sidebar.button("Run Analysis"):
        with st.spinner("Training Prophet model..."):
            ...
            st.download_button("Download Forecast CSV", forecast_csv, "prophet_forecast.csv", "text/csv")
    else:
